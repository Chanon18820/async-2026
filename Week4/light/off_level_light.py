...
async def turn_off_sequential(client: httpx.AsyncClient):
    """Turn off light_1, light_2, light_3, light_4 IN ORDER.
    Same idea as turn_on_sequential(): each light must finish
    before the next one starts. Total time = sum of all delays (~4.5s)."""
    print(f"{ctime()} | --- Turning OFF lights ONE BY ONE, in order 1 -> 2 -> 3 -> 4 ---")
    start_time = time()

    results = []
    for light_id in LIGHT_IDS:
        result = await turn_off_light(client, light_id)
        results.append(result)

    elapsed = time() - start_time
    print(f"{ctime()} | All {len(results)} lights are now OFF (sequential).")
    print(f"{ctime()} | Elapsed time: {elapsed:.2f} seconds "
          f"(should be close to the SUM of all delays, ~4.5s).")
...
async def main():
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        await turn_off_sequential(client)
